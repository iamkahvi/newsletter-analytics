...
//
// Usage: bun run scripts/sync_links.ts
...
if (newLinks.length === 0) {
  ...
  const currentContent = (await Bun.file(LINK_LIST).text()).trimEnd();
  await Bun.write(LINK_LIST, currentContent + "\n" + newLinks.join("\n") + "\n");
